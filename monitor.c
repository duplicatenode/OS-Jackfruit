...
/* kernel >= 6.1 renamed del_timer_sync -> timer_delete_sync */
 ...
 * in a softirq context on kernels < 5.x but as a normal kthread
 * timer on modern kernels, and because get_task_mm() (called inside
 * get_rss_bytes) may sleep.  A mutex is safe here; a spinlock would
 * be wrong whenever any sleepable call is made while holding it.
 * ============================================================== */
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        if (req.soft_limit_bytes > req.hard_limit_bytes) {
            printk(KERN_WARNING
                   "[container_monitor] soft > hard for %s, rejecting\n",
                   req.container_id);
            return -EINVAL;
        }
